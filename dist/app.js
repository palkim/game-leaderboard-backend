...
app.post("/leaderboard/earn", async (req, res) => {
    try {
        ...
        const [players] = await db.query("SELECT id FROM players WHERE id = ?", [playerId]);
        if (players.length === 0) {
...
app.post("/player/create", async (req, res) => {
    try {
        ...
        // Check if player exists in MySQL
        const [existingPlayers] = await db.query("SELECT id FROM players WHERE name = ? AND country = ? AND country_code = ?", [name, country, countryCode]);
        if (existingPlayers.length > 0) {
            return res.status(409).json({ error: "Player already exists" });
        }
        // Insert player into MySQL
        const [result] = await db.query("INSERT INTO players (name, country, country_code) VALUES (?, ?, ?)", [name, country, countryCode]);
...
async function getPlayerDetails(playerId) {
    ...
    const [player] = await db.query("SELECT name, country, country_code FROM players WHERE id = ?", [playerId]);
    if (player.length === 0) {
        return { name: null, country: null, country_code: null };
    }
    return {
...
async function getSearchResults(players) {
    let includedPlayerIds = new Set();
    const searchResults = await Promise.all(players.map(async (player) => {
        if (includedPlayerIds.has(player.id.toString()))
            return null;
        ...
        if (rank === null) {
            console.error(`Inconsistency detected: Player ${player.id} exists in User DB but not in Leaderboard.`);
            return null;
        }
        ...
        let prevPlayers = [];
        let nextPlayers = [];
        await Promise.all(surroundingPlayers.map(async (player, index) => {
            ...
            if (playerDetails.name === null ||
                ...
                console.error(`Inconsistency detected: Player with ID ${player.value} exists in Leaderboard but corrupted in User DB.`);
                return;
            }
...
app.get("/leaderboard/top-ranking-data", async (req, res) => {
    try {
        const { query } = req.query;
        const topRankingPlayers = await getTopRankingPlayers();
        if (!query)
            return res.json({ topRankingPlayers, searchResults: null });
        const [players] = await db.query(`SELECT id, name, country, country_code FROM players WHERE name LIKE ?`, [`%${query}%`]);
        if (!players.length)
            return res.json({ topRankingPlayers, searchResults: null });
        ...
        const filteredPlayers = playersWithRanks.filter((player) => player !== null);
