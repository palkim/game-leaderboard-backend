...
router.post('/leaderboard/earn', async (req: Request, res: Response) => {
    try {
...
router.get('/leaderboard/top100', async (req: Request, res: Response) => {
    try {
        const leaderboard = await redisClient.zRangeWithScores(LEADERBOARD_KEY, 0, 99, { REV: true });
...
router.get('/leaderboard/grouped', async (req: Request, res: Response) => {
    try {
        const leaderboard = await redisClient.zRangeWithScores(LEADERBOARD_KEY, 0, 99, { REV: true });
        res.json(leaderboard);
    } catch (error) {
...
router.get('/leaderboard/search', async (req: Request, res: Response) => {
    try {
